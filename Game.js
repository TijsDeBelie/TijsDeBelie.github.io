...
function checkAlive() {
	...
	if (execute = true) {
		if (hero.Health <= 0) {
			document.body.innerHTML = "<br>GAME OVER <br>" + Points + ' punten' + '<br><button class="startknop" onclick="location.reload();">Restart game</button>';
			pain.volume = 0;
            fire.volume = 0;
            hurt.volume = 0;
            heal.volume = 0;
...
function createEnemy() {
	...
	if (vijanden.length <= MaxEnemy) {
		...
		if (PosX < window.innerWidth && PosX > (50 + offsetPlayerW) && PosY > offsetPlayerH && PosY < window.innerHeight - 100) {
			vijanden.push(new Vijand(100, PosX, PosY));
		} else {
			PosX = Math.floor(Math.random() * window.innerWidth) + offsetPlayerW;
			PosY = Math.floor(Math.random() * window.innerHeight - 100) + offsetPlayerH;
			if (PosX < window.innerWidth && PosX > (50 + offsetPlayerW) && PosY > (0 + offsetPlayerH) && PosY < window.innerHeight - 100) {
				vijanden.push(new Vijand(100, PosX, PosY));
			}
		}
